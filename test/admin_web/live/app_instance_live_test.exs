defmodule AdminWeb.AppInstanceLiveTest do
  ...
  @create_attrs %{name: "some name", description: "some description", url: "some url", thumbnail: "some thumbnail"}
  ...
  describe "Index" do
    ...
    test "lists all apps", %{conn: conn, app_instance: app_instance} do
    ...
    test "saves new app_instance", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/apps")

      assert {:ok, form_live, _} =
               index_live
               |> element("a", "New App instance")
               |> render_click()
               |> follow_redirect(conn, ~p"/apps/new")

      assert render(form_live) =~ "New App instance"
    ...
    test "updates app_instance in listing", %{conn: conn, app_instance: app_instance} do
      {:ok, index_live, _html} = live(conn, ~p"/apps")
      ...
      assert {:ok, form_live, _html} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"/apps/#{app_instance}/edit")
  ...
  describe "Show" do
    ...
    test "updates app_instance and returns to show", %{conn: conn, app_instance: app_instance} do
      ...
      assert {:ok, form_live, _} =
               ...
               |> element("a", "Edit")
               |> render_click()
               |> follow_redirect(conn, ~p"/apps/#{app_instance}/edit?return_to=show")
