defmodule AdminWeb.UserSessionControllerTest do
  ...
  describe "POST /users/log-in - email and password" do
    test "logs the user in", %{conn: conn, user: user} do
      ...
      conn =
        post(conn, ~p"/admin/users/log-in", %{
      ...
      assert response =~ ~p"/admin/users/log-out"
    end
    ...
    test "redirects to login page with invalid credentials", %{conn: conn, user: user} do
      conn =
        post(conn, ~p"/admin/users/log-in?mode=password", %{
  ...
  describe "DELETE /users/log-out" do
    test "logs the user out", %{conn: conn, user: user} do
      conn = conn |> log_in_user(user) |> delete(~p"/admin/users/log-out")
    ...
    test "succeeds even if the user is not logged in", %{conn: conn} do
      conn = delete(conn, ~p"/admin/users/log-out")
      assert redirected_to(conn) == ~p"/admin"
      refute get_session(conn, :user_token)
      assert Phoenix.Flash.get(conn.assigns.flash, :info) =~ "Logged out successfully"
    end
